...
bool HttpRequest::parse(Buffer& buff) {
    ...
    while(buff.WritableBytes() > 0 && state_ != FINISH) {
        const char* lineEnd = search(buff.GetWritePos(), buff.GetReadPosConst(), CRLF, CRLF + 2);
        string line(buff.GetWritePos(), lineEnd);
        switch(state_) {
            case REQUEST_LINE:
